...
- Sequential-per-chunk fused matmul-epilogue at D=128 (iter24 — the "5 separate WGMMA sequences each with their own acc-zero / K-loop / spill" pattern paid more in per-chunk pipeline-fill overhead (~3 µs/chunk) than the saved `proj` HBM bandwidth. Net **+38–48% slower** on D=128 shapes. Only worth retrying with **interleaved-K** scheduling — i.e. all 5 N-chunks share one tight K-iter loop, accumulators kept alive across chunks — which is essentially iter18 Phase B's 5-acc cliff in a different guise. The C3 register limit makes it infeasible for BM ≥ 64 with hd=128.)
- Decomposed 2+2+1 fusion at D=384 (iter25 — fused matmul_L/R/og kernels are **64% slower standalone** than the existing `tlx_ws_matmul_fixed + fused_gate_ln_bmm_layout` pair; +20-24% e2e regression on D=384 shapes. Two wheel-level blockers: `tl.split` segfaults Triton AST visitor on the [BM_SPLIT, 2H] reshape, AND `tlx.local_slice` C++ `create_memdesc_subslice` binding has signature mismatch (the same broken API iter24 hit). Both forced design choices that doubled WGMMA cost; transpose-store-to-bmm-layout costs ~600 µs/kernel. Even if both wheel issues were fixed the theoretical ceiling is ~+200 µs (3-4% e2e). **Same class as C1/iter21/iter24 — wheel pybind/codegen limitations.**)
