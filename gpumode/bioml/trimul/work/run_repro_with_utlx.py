...
print(
    f"[wrap] median: {ms:.4f} ms   {tflops:.2f} TF/s   {gbs:.1f} GB/s", file=sys.stderr
)
print("[wrap] roofline: peak bf16 989 TF/s, peak HBM 3350 GB/s")
print(
    f"[wrap]           tflops/peak = {tflops / 989 * 100:.1f}%   bw/peak = {gbs / 3350 * 100:.1f}%"
)
print(
    f"[wrap] arithmetic intensity = {flops / bytes_total:.2f} FLOP/byte (H100 ridge ≈ 295 FLOP/byte for bf16)"
)
