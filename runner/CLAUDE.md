...
| `make_tensor_descriptor`       | `desc_ptr must be None or tlx.tensor_descriptor_ptr, got <class 'triton.language.core.constexpr'>`; or 5-vs-6-arg overload mismatch; or `'ttng.async_tma_copy_global_to_local' op TMA descriptor layout must match shared layout, but got descriptor layout <<NULL ATTRIBUTE>>` | `tlx.make_tensor_descriptor` (esp. with TMA loads) | utlx-py |
